...
func (s *ProxyHappySuite) TestPingErrorPropagatesAppError() {
	...
	assert.Equal(s.T(), codes.FailedPrecondition, st.Code())//grpc.Code(err))
	assert.Equal(s.T(), "Userspace error.", st.Message())//grpc.ErrorDesc(err))
}
...
func (s *ProxyHappySuite) TestDirectorErrorIsPropagated() {
	...
	st, _ := status.FromError(err)
	assert.Equal(s.T(), codes.PermissionDenied, st.Code())//grpc.Code(err))
	assert.Equal(s.T(), "testing rejection", st.Message())//grpc.ErrorDesc(err))
}
...
func (s *ProxyHappySuite) SetupSuite() {
	...
	lv2 := grpclog.NewLoggerV2(os.Stderr, os.Stderr, os.Stderr)
	grpclog.SetLoggerV2(lv2)//log.New(os.Stderr, "grpc: ", log.LstdFlags))

	s.server = grpc.NewServer()
	pb.RegisterTestServiceServer(s.server, &assertingService{t: s.T()})

	// Setup of the proxy's Director.
	s.serverClientConn, err = grpc.Dial(s.serverListener.Addr().String(), grpc.WithInsecure(), grpc.WithDefaultCallOptions(grpc.CallCustomCodec(Codec())))
	...
	//DialContext and context.WithTimeout
